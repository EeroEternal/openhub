...
export default function ForgotPage() {
  ...
  async function onSendCode(e?: FormEvent) {
    ...
    setSendingCode(true)
    try {
      await api<{ ok: boolean }>("/api/v1/auth/forgot/send-code", {
        method: "POST",
  ...
  async function onVerifyCode(e: FormEvent) {
    ...
    setPending(true)
    try {
      await api<{ ok: boolean }>("/api/v1/auth/forgot/verify-code", {
        method: "POST",
  ...
  async function onReset(e: FormEvent) {
    ...
    setPending(true)
    try {
      const res = await api<{ ok: boolean; token?: string }>("/api/v1/auth/forgot/reset", {
        method: "POST",
        body: JSON.stringify({
          email: email.trim(),
          code: code.trim(),
  ...
  return (
    <AuthCardLayout activeTab="login" title={t("auth.forgotTitle")} showSwitch={false}>
      ...
      <p className="mt-6 text-center text-meta-sm text-muted-foreground">
